...
class Trainer:
    ...
    def train(self, epoch):
        ...
        for batch_idx, (inputs, targets, idx) in enumerate(process):
            ...
            with torch.no_grad():
                ...
                # Get linear acceleration data if available
                linear_acceleration = None
                if 'linear_acceleration' in inputs:
                    linear_acceleration = inputs['linear_acceleration'].to(device)
            ...
            if linear_acceleration is not None and quaternion is not None:
                # Handle fusion transformer model
                data_dict = {
                    'linear_acceleration': linear_acceleration.float(),
                    'quaternion': quaternion.float()
                }
                logits = self.model(data_dict)
            elif fusion_features is not None and hasattr(self.model, 'forward_fusion'):
                # Model has specialized fusion handling
                logits = self.model.forward_fusion(acc_data.float(), fusion_features.float())
            elif quaternion is not None and hasattr(self.model, 'forward_quaternion'):
    ...
    def eval(self, epoch, loader_name='val', result_file=None):
        ...
        if loader_name in ['val', 'test']:
            with open(f'{self.arg.work_dir}/{loader_name}_result.txt', 'w') as f:
