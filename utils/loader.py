...
class FallDetectionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        acc_data = torch.tensor(self.accelerometer[idx], dtype=torch.float32)
...
def prepare_datasets(args, fold_idx=0):
    ...
    try:
        ...
        train_set = FallDetectionDataset(train_data)
        val_set = FallDetectionDataset(val_data) 
        test_set = FallDetectionDataset(test_data)
...
def create_data_loaders(train_set, val_set, test_set, batch_size, test_batch_size, num_workers):
    ...
    val_loader = DataLoader(val_set, batch_size=test_batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
    test_loader = DataLoader(test_set, batch_size=test_batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
