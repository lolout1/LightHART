...
def get_filter(filter_id, filter_type='madgwick', params=None, create_if_missing=True, reset=False):
    ...
    elif filter_type == 'kalman': filter_instance = KalmanFilter()
    elif filter_type == 'ekf': filter_instance = ExtendedKalmanFilter()
    elif filter_type == 'ukf': filter_instance = UnscentedKalmanFilter()
    elif filter_type == 'comp': filter_instance = ComplementaryFilter()
...
def align_sensor_data(acc_data, gyro_data, time_tolerance=0.01):
    ...
    # Handle numpy array case (already aligned data)
    ...
    # Use hybrid interpolation for both sensors
...
def process_imu_data(acc_data, gyro_data, timestamps=None, filter_type='madgwick', 
    ...
    for i in range(len(acc_data)):
        ...
        # Calculate gravity direction based on previous orientation
        ...
        # Normalize accelerometer with gravity
...
def extract_features(acc_data, gyro_data, quat_data):
    ...
    # Convert quaternions to Euler angles for orientation features
