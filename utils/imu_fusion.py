...
# Function to process multiple files in parallel using the thread pool
def process_files_parallel(file_paths, process_func, *args, **kwargs):
    """
    ...
    Returns:
        List of results in the same order as file_paths
    """
    ...
    for i, file_path in enumerate(file_paths):
        future = thread_pool.submit(process_func, file_path, *args, **kwargs)
    ...
    with tqdm(total=len(file_paths), desc="Processing files") as pbar:
        for i, future in futures:
            try:
                ...
                logger.error(f"Error processing file {file_paths[i]}: {str(e)}")
                results[i] = None
            pbar.update(1)
...
# Add a function to clean up resources when done
