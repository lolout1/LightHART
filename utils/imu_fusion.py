...
def register_filter(filter_id, filter_instance):
    with filter_locks[filter_id]: filter_registry[filter_id] = filter_instance
...
def clear_filters():
    for filter_id in list(filter_registry.keys()):
        with filter_locks[filter_id]:
            if filter_id in filter_registry: del filter_registry[filter_id]

def robust_interpolate(x, y, x_new):
    if len(x) < 2 or len(y) < 2: return np.full_like(x_new, y[0] if len(y) > 0 else 0.0)
    try:
        rates = np.abs(np.diff(y) / np.maximum(np.diff(x), 1e-10))
        rapid_changes = rates > 2.0
        if not np.any(rapid_changes):
            try: return interp1d(x, y, kind='cubic', bounds_error=False, fill_value='extrapolate')(x_new)
...
def align_sensor_data(acc_data, gyro_data, time_tolerance=0.01):
    ...
    if isinstance(acc_data, np.ndarray) and isinstance(gyro_data, np.ndarray):
        ...
        try:
            if isinstance(acc_data.iloc[0, 0], str): acc_times = pd.to_datetime(acc_data.iloc[:, 0]).astype(np.int64) // 10**6
            else: acc_times = acc_data.iloc[:, 0].values
            if isinstance(gyro_data.iloc[0, 0], str): gyro_times = pd.to_datetime(gyro_data.iloc[:, 0]).astype(np.int64) // 10**6
    ...
    if end_time <= start_time: return np.array([]), np.array([]), np.array([])
    ...
    if len(common_times) < 5: return np.array([]), np.array([]), np.array([])
    
    acc_in_range = (acc_times >= start_time) & (acc_times <= end_time)
    gyro_in_range = (gyro_times >= start_time) & (gyro_times <= end_time)
    
    if np.sum(acc_in_range) < 3 or np.sum(gyro_in_range) < 3: return np.array([]), np.array([]), np.array([])
    
    aligned_acc = np.zeros((len(common_times), 3))
    aligned_gyro = np.zeros((len(common_times), 3))
    
    for axis in range(3):
        aligned_acc[:, axis] = robust_interpolate(acc_times[acc_in_range], acc_values[acc_in_range, axis], common_times)
        aligned_gyro[:, axis] = robust_interpolate(gyro_times[gyro_in_range], gyro_values[gyro_in_range, axis], common_times)
...
def process_window_with_filter(acc_window, gyro_window, timestamps=None, filter_type='madgwick', 
    ...
    if acc_window.shape[0] == 0 or gyro_window.shape[0] == 0: return {'quaternion': np.zeros((1, 4))}
    ...
    if timestamps is None: timestamps = np.linspace(0, acc_window.shape[0] / 30.0, acc_window.shape[0])
    if filter_id is None: filter_id = f"window_{filter_type}_{np.random.randint(0, 10000)}"
...
def process_sequential_windows(data, window_size=64, stride=32, label=None, filter_type='madgwick', 
    ...
    """Process data in sequential windows with filter state preservation"""
    print(f"Processing sequential windows: stateful={stateful}, filter_type={filter_type}")
    
    if 'accelerometer' not in data or data['accelerometer'] is None or 'gyroscope' not in data or data['gyroscope'] is None:
        print("Missing accelerometer or gyroscope data, skipping")
    ...
    print(f"Data shapes: acc={acc_data.shape}, gyro={gyro_data.shape}")
    
    if len(acc_data) < window_size // 2:
        print(f"Data too short: {len(acc_data)} samples < {window_size//2}, skipping")
        return {'labels': np.array([label])} if label is not None else {'labels': np.array([])}
    ...
    # Ensure we have a valid filter_id
    filter_id = base_filter_id if base_filter_id is not None else f"sequential_{filter_type}"
    print(f"Processing {num_windows} sequential windows with filter_id={filter_id}")
    
    # Get or create filter instance (don't reset if stateful)
    params = filter_params or {}
    filter_instance = get_filter(filter_id, filter_type, params=params, reset=not stateful)
    
    valid_window_count = 0
    
    # Process each window sequentially, maintaining filter state
    for i in range(num_windows):
        ...
        if end - start < window_size // 2:
            print(f"Window {i+1}/{num_windows} too short ({end-start} < {window_size//2}), skipping")
            continue
        ...
        # Skip if no gyro data for this window
        if actual_gyro_length <= 0:
            print(f"Window {i+1}/{num_windows} has no gyro data, skipping")
            continue
        ...
        try:
            # Process window with filter, don't reset filter between windows
            ...
            result = process_window_with_filter(
                ...
                reset_filter=False,  # Never reset filter between windows
            ...
            print(f"Window {i+1}/{num_windows} processed successfully")
        except Exception as e:
            print(f"Failed to process window {i+1}/{num_windows}: {e}")
            continue
    ...
    print(f"Processed {valid_window_count} valid windows")
    return windows
...
def align_sequence(data):
    ...
    if acc_data is None or gyro_data is None or len(acc_data) == 0 or len(gyro_data) == 0: return data
