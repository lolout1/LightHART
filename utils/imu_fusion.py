...
def hybrid_interpolate(x, y, x_new, threshold=2.0, window_size=5):
    ...
    try:
        ...
        segments, segment_start, buffer = [], None, 0.05
...
def align_sensor_data(acc_data, gyro_data, time_tolerance=0.01):
    ...
    if isinstance(acc_data, np.ndarray) and isinstance(gyro_data, np.ndarray):
        ...
        try:
            ...
            acc_values, gyro_values = acc_data.iloc[:, 1:4].values, gyro_data.iloc[:, 1:4].values
        except: return np.array([]), np.array([]), np.array([])
    start_time, end_time = max(acc_times[0], gyro_times[0]), min(acc_times[-1], gyro_times[-1])
    acc_sampling, gyro_sampling = np.median(np.diff(acc_times)), np.median(np.diff(gyro_times))
    target_sampling = min(acc_sampling, gyro_sampling)
    common_times = np.array(sorted(set(acc_times).union(set(gyro_times))))
    common_times = common_times[(common_times >= start_time) & (common_times <= end_time)]
    if len(common_times) > 3 * max(len(acc_times), len(gyro_times)): common_times = np.arange(start_time, end_time, target_sampling)
    ...
    for axis in range(3):
        aligned_acc[:, axis] = hybrid_interpolate(acc_times[acc_in_range], acc_values[acc_in_range, axis], common_times, threshold=2.0, window_size=5)
        aligned_gyro[:, axis] = hybrid_interpolate(gyro_times[gyro_in_range], gyro_values[gyro_in_range, axis], common_times, threshold=1.0, window_size=5)
...
class MadgwickFilter(OrientationFilter):
    ...
    def _update_impl(self, acc, gyro, dt):
        ...
        J_t = np.array([[-2.0*q2, 2.0*q3, -2.0*q0, 2.0*q1], [2.0*q1, 2.0*q0, 2.0*q3, 2.0*q2], [0.0, -4.0*q1, -4.0*q2, 0.0]])
        grad = J_t.T @ np.array([f1, f2, f3])
        if (grad_norm := np.linalg.norm(grad)) > 0: grad = grad / grad_norm
        qDot = 0.5 * np.array([-q1*gyro[0]-q2*gyro[1]-q3*gyro[2], q0*gyro[0]+q2*gyro[2]-q3*gyro[1], q0*gyro[1]-q1*gyro[2]+q3*gyro[0], q0*gyro[2]+q1*gyro[1]-q2*gyro[0]])
...
class ExtendedKalmanFilter(OrientationFilter):
    ...
    def _measurement_jacobian(self, q):
        ...
        H_acc[:3, :4] = np.array([[2*y, 2*z, 2*w, 2*x], [-2*z, 2*y, 2*x, -2*w], [-2*x, -2*w, 2*z, 2*y]])
        return H_acc

def process_window_with_filter(acc_window, gyro_window, timestamps=None, filter_type='madgwick', 
                             filter_id=None, reset_filter=False, is_linear_acc=True):
    ...
    if filter_id is None: filter_id = f"window_{filter_type}"
