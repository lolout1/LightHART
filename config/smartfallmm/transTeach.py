...
class TeacherTransformerFusion(nn.Module):
    ...
    def __init__(
        ...
        self.watch_pos = nn.Parameter(torch.randn(1, 64, d_model))
        self.skel_pos  = nn.Parameter(torch.randn(1, 64, d_model))
    ...
    def forward(self, watch_data, skeleton_data):
        """
        watch_data: [B, 64, 3] (accelerometer x,y,z)
        skeleton_data: [B, 64, 32, 3] (aligned skeleton frames)
        Returns:
          teacher_logits: [B], raw
          teacher_feat:   [B, d_model]
        """
        B, T, _ = watch_data.shape

        # 1) Add watch magnitude => shape [B, 64, 4]
        mag = self._compute_magnitude(watch_data)
        watch_in = torch.cat([watch_data, mag], dim=-1)  # => [B, T, 4]

        # 2) Flatten skeleton => shape [B, T, 96]
        skel_in = skeleton_data.view(B, T, -1)  # => [B, T, 96]
