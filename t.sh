...
def create_comparison_report(df, output_path):
    ...
    with open(output_path, 'w') as f:
        ...
        for filter_type in df['filter_type']:
            ...
            f.write("\n")
        f.write("\n## Filter Descriptions\n\n")
        f.write("- **Madgwick**: A computationally efficient orientation filter that uses gradient descent.\n")
        f.write("- **Kalman**: Standard Kalman filter implementation that optimally combines sensor data.\n")
        f.write("- **EKF (Extended Kalman Filter)**: Advanced Kalman filter for non-linear relationships.\n")
