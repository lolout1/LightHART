# File: Models/t3.py
import torch
import torch.nn as nn
import torch.nn.functional as F
from utils.processor.base import Time2Vec

def masked_mean(features, mask):
    """
    Computes the mean over the time dimension for each sample, ignoring padded positions.
    mask: boolean tensor with True for padded positions => padded.
    """
    if mask is not None:
        valid = ~mask
        features = features * valid.unsqueeze(-1).float()
        return features.sum(dim=1) / valid.sum(dim=1).clamp(min=1).unsqueeze(-1)
    else:
        return features.mean(dim=1)

class TransformerTeacher(nn.Module):
    def __init__(
        self,
        num_joints=32,
        joint_dim=3,
        hidden_skel=128,   # final dimension of skeleton branch
        accel_dim=3,
        time2vec_dim=8,   # NOTE: 8 to match your logged teacher
        hidden_accel=64,  # final dimension of accelerometer branch
        accel_heads=4,
        accel_layers=3,
        skeleton_heads=4,
        skeleton_layers=2,
        fusion_hidden=128,   # teacher final "fusion" dimension
        num_classes=2,
        dropout=0.2,         # match your logs
        dim_feedforward=128, # feedforward dimension in transformer
        **kwargs
    ):
        """
        Teacher model using skeleton + watch accelerometer data.
        Returns:
            {
              "logits": <B, num_classes>,
              "features": {
                  "accel_seq": <B, T, hidden_accel>,
                  "fusion": <B, fusion_hidden>,
                  "skel_seq": <B, T, hidden_skel>,
              }
            }
        """
        super().__init__()

        # (A) Skeleton Transformer Branch
        self.skel_embed = nn.Linear(num_joints * joint_dim, hidden_skel)
        self.skel_pos = nn.Parameter(torch.randn(1, 120, hidden_skel))  # up to 120 frames

        skel_layer = nn.TransformerEncoderLayer(
            d_model=hidden_skel,
            nhead=skeleton_heads,
            dim_feedforward=dim_feedforward,
            dropout=dropout,
            activation='gelu',
            batch_first=True
        )
        self.skel_transformer = nn.TransformerEncoder(skel_layer, num_layers=skeleton_layers)

        # (B) Accelerometer Branch
        self.time2vec = Time2Vec(out_channels=time2vec_dim)
        # 3 accel dims + 8 T2V => 11 => project to hidden_accel=64
        self.accel_proj = nn.Linear(accel_dim + time2vec_dim, hidden_accel)

        accel_layer = nn.TransformerEncoderLayer(
            d_model=hidden_accel,
            nhead=accel_heads,
            dim_feedforward=dim_feedforward * 2,  # more capacity
            dropout=dropout,
            activation='gelu',
            batch_first=True
        )
        self.accel_transformer = nn.TransformerEncoder(accel_layer, num_layers=accel_layers)

        # (C) Fusion => final dimension = 128
        self.fusion = nn.Sequential(
            nn.Linear(hidden_skel + hidden_accel, fusion_hidden),
            nn.LayerNorm(fusion_hidden),
            nn.GELU(),
            nn.Dropout(dropout)
        )
        self.classifier = nn.Linear(fusion_hidden, num_classes)

    def forward(self, skel_seq, accel_seq, accel_time, accel_mask=None):
        B, Ts, _ = skel_seq.shape

        # SKELETON
        skel_emb = self.skel_embed(skel_seq) + self.skel_pos[:, :Ts, :]
        skel_seq_feat = self.skel_transformer(skel_emb)  # (B, Ts, hidden_skel)
        skel_feat = skel_seq_feat.mean(dim=1)

        # ACCEL
        B, Ta, _ = accel_seq.shape
        t_emb = self.time2vec(accel_time.reshape(-1, 1))
        t_emb = t_emb.view(B, Ta, -1)
        accel_in = torch.cat([accel_seq, t_emb], dim=-1)  # shape=(B, Ta, 3+8=11)
        accel_in = F.gelu(self.accel_proj(accel_in))
        accel_seq_feat = self.accel_transformer(accel_in, src_key_padding_mask=accel_mask)
        accel_feat = masked_mean(accel_seq_feat, accel_mask)

        # FUSION
        fused_feat = self.fusion(torch.cat([skel_feat, accel_feat], dim=-1))  # => [B, 128]
        logits = self.classifier(fused_feat)

        return {
            "logits": logits,
            "features": {
                "accel_seq": accel_seq_feat,
                "fusion": fused_feat,
                "skel_seq": skel_seq_feat
            }
        }

