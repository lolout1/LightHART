...
class FusionTransModel(nn.Module):
    """
    Optimized transformer-based model for IMU sensor fusion and fall detection.
    
    This model supports flexible modality combinations with configurable weights:
    - Accelerometer only
    - Accelerometer + Quaternion (orientation from fusion filters)
    - Accelerometer + Gyroscope
    - Accelerometer + Gyroscope + Quaternion
    
    Features:
    - Modality-specific encoders optimized for each sensor type
    - Configurable modality weights for emphasis on most reliable signals
    - Multiple fusion strategies (concat, attention, weighted)
    ...
    def __init__(self,
                ...
                feature_dim=None,
                use_gyro=True,
                acc_weight=1.5,  # Increased weight for accelerometer (more reliable)
                gyro_weight=0.8, # Decreased weight for gyroscope
        ...
        # Create accelerometer encoder with larger kernels for better temporal feature extraction
        self.acc_encoder = nn.Sequential(
            # First layer - extract low-level features
            nn.Conv1d(acc_coords, embed_dim, kernel_size=5, padding='same'),
            nn.BatchNorm1d(embed_dim) if use_batch_norm else nn.Identity(),
            nn.GELU(),
            nn.Dropout(dropout/2),
            
            # Second layer - extract temporal patterns
            nn.Conv1d(embed_dim, embed_dim, kernel_size=7, padding='same'),
            nn.BatchNorm1d(embed_dim) if use_batch_norm else nn.Identity(),
            nn.GELU(),
            
            # Optional third layer for complex feature extraction
            nn.Conv1d(embed_dim, embed_dim, kernel_size=3, padding='same'),
            nn.BatchNorm1d(embed_dim) if use_batch_norm else nn.Identity(),
            nn.GELU()
        )

        # Gyroscope encoder (optimized for angular velocity patterns)
        if use_gyro:
        ...
        # Quaternion encoder (specialized for orientation representation)
        ...
        # Create transformer encoder with enhanced attention mechanism
        ...
        # Improved classification head with skip connection
    ...
    def forward_acc_quat(self, acc_data, quat_data):
        ...
        """
        try:
            # Process accelerometer data with enhanced temporal feature extraction
    ...
    def forward(self, data):
        ...
        try:
            if isinstance(data, dict):
                ...
                # Remove redundant linear_acceleration from processing
                if 'linear_acceleration' in data:
                    logger.debug("Ignoring redundant linear_acceleration as accelerometer data already represents linear acceleration")
