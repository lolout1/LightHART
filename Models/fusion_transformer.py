...
class EnhancedFusionModule(nn.Module):
    ...
    def forward(self, acc_features, gyro_features, quat_features):
        enhanced_acc, _ = self.cross_attention(
            acc_features,
            torch.cat([gyro_features, quat_features], dim=1),
            torch.cat([gyro_features, quat_features], dim=1)
        )
        
        batch_size, seq_len, _ = acc_features.shape
        concat_features = torch.cat([
            acc_features.mean(dim=1),
            gyro_features.mean(dim=1),
            quat_features.mean(dim=1)
        ], dim=-1)
        weights = self.fusion_gate(concat_features).unsqueeze(1).expand(-1, seq_len, -1)
        
        fused_features = (
            weights[:, :, 0:1] * enhanced_acc +
            weights[:, :, 1:2] * gyro_features +
            weights[:, :, 2:3] * quat_features
        )
        
        output = self.feature_enhance(fused_features)
...
class FusionTransModel(nn.Module):
    def __init__(self,
        ...
        self.filter_type = kwargs.get('filter_type', 'ekf')
        ...
        self.lin_acc_encoder = nn.Sequential(
            nn.Conv1d(acc_coords, embed_dim, kernel_size=3, padding='same'),
            nn.BatchNorm1d(embed_dim) if use_batch_norm else nn.Identity(),
            nn.GELU(),
            nn.Dropout(dropout/2),
        ...
        if fusion_type == 'enhanced':
            self.fusion_module = EnhancedFusionModule(embed_dim, num_heads=num_heads)
        ...
        logger.info(f"Initialized FusionTransModel: fusion={fusion_type}, filter={self.filter_type}")

    def forward_all_modalities(self, acc_data, gyro_data, quat_data=None, linear_acc_data=None):
        try:
            ...
            has_quat = quat_data is not None and not torch.all(quat_data == 0)
            has_lin_acc = linear_acc_data is not None and not torch.all(linear_acc_data == 0)
            ...
            if has_lin_acc:
                lin_acc_data = rearrange(linear_acc_data, 'b l c -> b c l')
                lin_acc_features = self.lin_acc_encoder(lin_acc_data)
                lin_acc_features = rearrange(lin_acc_features, 'b c l -> b l c')
            else:
                lin_acc_features = acc_features
            
            if self.fusion_type == 'enhanced':
                fused_features = self.fusion_module(acc_features, gyro_features, quat_features)
            elif self.fusion_type == 'gated':
                batch_size, seq_len = acc_features.shape[0], acc_features.shape[1]
                acc_gate = torch.sigmoid(self.acc_gate(acc_features.mean(dim=1))).view(batch_size, 1, 1)
                gyro_gate = torch.sigmoid(self.gyro_gate(gyro_features.mean(dim=1))).view(batch_size, 1, 1)
                quat_gate = torch.sigmoid(self.quat_gate(quat_features.mean(dim=1))).view(batch_size, 1, 1)
                
                fused_features = (acc_gate * acc_features + 
                                 gyro_gate * gyro_features + 
                                 quat_gate * quat_features) / (acc_gate + gyro_gate + quat_gate + 1e-6)
            elif self.fusion_type == 'concat':
                if has_lin_acc:
                    fused_features = torch.cat([acc_features, gyro_features, lin_acc_features], dim=2)
                else:
    ...
    def forward(self, data):
        try:
            if isinstance(data, dict):
                ...
                has_lin_acc = 'linear_acceleration' in data and data['linear_acceleration'] is not None
                
                if not has_quaternion and has_acc and has_gyro:
                    logger.warning("Quaternion data missing. Adding empty quaternion tensor.")
                ...
                if has_acc and has_gyro and (has_quaternion or has_lin_acc):
                    return self.forward_all_modalities(
                        data['accelerometer'],
                        data['gyroscope'],
                        data.get('quaternion'),
                        data.get('linear_acceleration')
                    )
                    ...
                    logger.info("Missing gyroscope data, using accelerometer only")
