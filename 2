...
class Trainer():
    def __init__(self, arg):
        ...
        self.early_stop = EarlyStopping(patience=15, min_delta=.001, monitor='recall')
    ...
    def cal_weights(self):
        ...
        self.pos_weights = self.pos_weights.to(f'cuda:{self.output_device}' if torch.cuda.is_available() else 'cpu')
    ...
    def load_optimizer(self, parameters):
        if self.arg.optimizer.lower() == "adam":
            self.optimizer = optim.Adam(parameters, lr=self.arg.base_lr)
        elif self.arg.optimizer.lower() == "adamw":
            self.optimizer = optim.AdamW(parameters, lr=self.arg.base_lr, weight_decay=self.arg.weight_decay)
        elif self.arg.optimizer.lower() == "sgd":
            self.optimizer = optim.SGD(parameters, lr=self.arg.base_lr)
        else:
            raise ValueError()
    ...
    def train(self, epoch):
        ...
        self.print_log(
            '\tTraining Loss: {:4f},  Acc: {:2f}%, F1 score: {:2f}%, Precision: {:2f}%, Recall: {:2f}%, AUC: {:2f}%  '.format(
                train_loss, accuracy, f1, precision, recall, auc_score))
    ...
    def eval(self, epoch, loader_name='val', result_file=None):
        ...
        self.print_log('{} Loss: {:4f}. {} Acc: {:2f}% F1 score: {:2f}%, Precision: {:2f}%, Recall: {:2f}%, AUC: {:2f}%'.format(
            loader_name.capitalize(), loss, loader_name.capitalize(), accuracy, f1, precision, recall, auc_score))
